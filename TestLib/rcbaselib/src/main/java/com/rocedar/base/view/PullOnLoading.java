 ...
 * 加载更多处理
 * <p/>
 * GridView、ScrollView 使用该类需要在XML中inCloud ‘view_loadmore.xml’
 * ListView  不用inCloud
 * <p/>
...
public class PullOnLoading {
    ...
    public PullOnLoading(Activity context, ScrollView scrollView) {
        ...
        try {
            ...
            RCLog.e("PullOnLoading", "----scrollView没有inCloud ‘view_loadmore.xml-----");
        }
    ...
    public PullOnLoading(Activity context, View view, ListView mListView,
        ...
        if (view == null)
            bottomView = LayoutInflater.from(mContext).inflate(R.layout.view_loadmore, null);
        else
    ...
    public PullOnLoading(Activity mContext, View view, final GridView gridView,
        ...
        try {
            ...
            RCLog.e("PullOnLoading", "----gridView没有inCloud ‘view_loadmore.xml-----");
        }
    ...
    private void loadStart() {
        ...
        loading = true;
        if (loadMore != null)
            loadMore.setText(mContext.getString(R.string.base_loading));
    ...
    public void loadOver(boolean hasMore) {
        if (!hasMore) {
            ...
            if (loadMore != null)
                loadMore.setText(mContext.getString(R.string.base_no_more));
            loading = true;
            return;
        }
        if (loadMore != null)
            loadMore.setText(mContext.getString(R.string.base_load_more));
        loading = false;
