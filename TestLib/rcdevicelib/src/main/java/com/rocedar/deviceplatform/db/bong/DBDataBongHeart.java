...
public class DBDataBongHeart implements IBongHeartInfo {
    ...
    public DBDataBongHeart(Context context) {
    ...
    public void addInfo(final BongInfoDTO infoNew) {
        ...
        try {
            ...
            values.put(BONG_HEARTINFO, infoNew.getInfo());
            if (temp >= 0) {
...
//                RCLog.d(TAG, "查询是否存在结果:" + temp);
    ...
    public List<BongInfoDTO> getInfoFromMacByTime(String mac, long startTime, long endTime) {
        if (!db.isOpen()) {
            return null;
        }
        ...
        while (cursor.moveToNext()) {
            ...
            dto.setInfo(cursor.getString(cursor.getColumnIndex(BONG_HEARTINFO)));
            bongList.add(dto);
        }
        cursor.close();
        return bongList;
