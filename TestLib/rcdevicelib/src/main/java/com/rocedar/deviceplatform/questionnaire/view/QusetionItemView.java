package com.rocedar.deviceplatform.questionnaire.view;
...
//                    viewTextLine.add(tempLine2);
    ...
    private List<HealthHistoryQuestionOptionsDTO> mOptionsDTOs;
...
//    private List<Integer> viewTextLine;
     ...
     * 设置数据源：数据源的字符串集合
     */
    public void setItemList(List<HealthHistoryQuestionOptionsDTO> optionsDTOs, int chooseType
            , String chooseD) {
...
//            textView.setLayoutParams(lp);
            ...
            textView.setBackgroundResource(R.drawable.rectangle_cccccc_5px);
            textView.setTextColor(getResources().getColor(R.color.rcbase_app_text_default));
            for (int optid : chooseNumber) {
                if (optid == mOptionsDTOs.get(i).getOption_id()) {
                    textView.setBackgroundResource(R.drawable.rectangle_main_5px);
                    textView.setTextColor(getResources().getColor(R.color.rcbase_white));
                }
            }
    ...
    private void reLoadTextViewBg(int chooseIndex) {
        if (chooseType == HealthHistoryData.Type.Radio) {
            ...
            for (int i = 0; i < mTextViewList.size(); i++) {
                if (i == chooseIndex) {
                    mTextViewList.get(i).setBackgroundResource(R.drawable.rectangle_main_5px);
                    mTextViewList.get(i).setTextColor(getResources().getColor(R.color.rcbase_white));
                } else {
                    mTextViewList.get(i).setBackgroundResource(R.drawable.rectangle_cccccc_5px);
                    mTextViewList.get(i).setTextColor(getResources().getColor(R.color.rcbase_app_text_default));
                }
            }
        } else if (chooseType == HealthHistoryData.Type.MultiSelect) {
            if (removeChoose(mOptionsDTOs.get(chooseIndex).getOption_id())) {
                //取消了选中
                mTextViewList.get(chooseIndex).setBackgroundResource(R.drawable.rectangle_cccccc_5px);
                mTextViewList.get(chooseIndex).setTextColor(getResources().getColor(R.color.rcbase_app_text_default));
            } else {
                ...
                chooseNumber.add(mOptionsDTOs.get(chooseIndex).getOption_id());
                mTextViewList.get(chooseIndex).setBackgroundResource(R.drawable.rectangle_main_5px);
                mTextViewList.get(chooseIndex).setTextColor(getResources().getColor(R.color.rcbase_white));
            }
        }
    }
