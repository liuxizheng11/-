...
public class Regix<T extends RCBean> {
    ...
    private static String[] fieldNames = {"actionName", "baseNetUrl", "networkMethod"};


    private static boolean checkName(String filedName) {
        for (int i = 0; i < fieldNames.length; i++) {
            if (filedName.equals(fieldNames[i])) {
                return false;
            }
        }
        return true;
    ...
    private SortedMap<String, String> getParams(T t) {
        ...
        for (Field fd : t.getClass().getFields()) {
            ...
            try {
                ...
                RCLog.w(TAG, filedName + "->null或者没有方法");
            }
        }
        return mSortedMap;
    }
     ...
     * 生成SignCode（密钥串）
     * 密钥生成规则(32位MD5串)：
     * MD5[请求类型大写（GET|POST|PUT|DELETE）+URL(去协议头)+参数按字母顺序排序后的字符串(汉字逐个Encode)+app指定混淆串]
     *
     * @param t
     * @param methodName
     * @return
    ...
    public String getSignCode(T t, String methodName) {
        ...
        if (t.getNetworkMethod() == NetworkMethod.SDK) {
            buffer.append(t.getBaseNetUrl().replace("http://", "").replace("https://", ""));
            buffer.append(t.getActionName());
        }
        buffer.append(postBody);
        buffer.append(t.getNetworkMethod().getSecretKey());
        ...
        RCLog.d(TAG, "密钥生成前的字符串(Encode)：" + sign);
     ...
     * @return
     */
    public String getSignCode(String url, NetworkMethod type) {
        Uri temp = Uri.parse(url);
        SortedMap<String, String> mSortedMap = new TreeMap<>();
        for (String str : temp.getQueryParameterNames()) {
            mSortedMap.put(str, temp.getQueryParameter(str));
        }
        String postBody = "";
        for (Map.Entry entry : mSortedMap.entrySet()) {
            if (null != entry.getValue()) {
                if (!entry.getValue().equals("@null")) {
                    postBody += entry.getKey() + "=" + entry.getValue() + "";
                } else {
        ...
        StringBuffer buffer = new StringBuffer();
        buffer.append("GET");
        if (type == NetworkMethod.SDK)
            buffer.append(temp.getHost() + (temp.getPort() > 0 ? ":" + temp.getPort() : "") + temp.getPath());
        buffer.append(postBody);
        buffer.append(type.getSecretKey());
        String sign = buffer.toString();
        RCLog.d(TAG, "密钥生成前的字符串：" + sign);
        try {
            sign = URLEncoder.encode(sign, "UTF-8");
