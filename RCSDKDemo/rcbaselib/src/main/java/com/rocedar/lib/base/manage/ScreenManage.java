...
public class ScreenManage {
    ...
    private ScreenManage() {
        OutAvtivityName.add(RCWebViewActivity.class.getName());
    }
    ...
    public void finishActivity(Class<?> cls) {
        boolean temp = true;
        while (temp && activityStack.size() > 0) {
            a:
    ...
    public boolean finishAllActivityAndToLogin(Class loginActivity) {
        try {
            while (activityStack.size() > 1) {
                if (null != activityStack.get(activityStack.size() - 2)) {
                    if (activityStack.get(activityStack.size() - 2).getClass().getName().contains(loginActivity.getName())) {
                        return true;
                    }
                    RCLog.d(TAG, "退出activityall->" + activityStack.get(activityStack.size() - 2).getLocalClassName());
