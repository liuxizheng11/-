...
public class FDIntroducedAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public FDIntroducedAdapter(
            Context context, List<RCFDDoctorCommentsDTO> familyDoctorCommentDTOs,
            RCFDDoctorIntroduceDTO introduceDTO) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        switch (viewType) {
            case TYPE_DOCTOR_INFO:
                return new DoctorInfoHolder(getView(parent, R.layout.rc_fd_activity_specialist_introduced_doctor_info));
            case TYPE_COMMENT_LIST:
                return new CommentHolder(getView(parent, R.layout.rc_fd_activity_specialist_introduced_comment_adapter));
            case TYPE_FOOTER:
                return new HolderFooter(LayoutInflater.from(parent.getContext()).inflate(R.layout.rc_view_loadmore, parent, false));
        }
        return null;
    }
    ...
    private void doTypeDoctorInfoHolder(DoctorInfoHolder holder) {
        ...
        //咨询次数
        holder.headNumber.setText(context.getString(R.string.rc_fd_consult_count_number,
        ...
        if (introduceDTO.getStatus() == RCFDDoctorListDTO.DOCTOR_STATUS_BUSY ||
            ...
            //在线
            holder.doctorStatus.setBackgroundResource(R.mipmap.rc_fd_ic_family_doctor_idle);
        } else if (introduceDTO.getStatus() == RCFDDoctorListDTO.DOCTOR_STATUS_OFFLINE) {//离线
            holder.doctorStatus.setBackgroundResource(R.mipmap.rc_fd_ic_family_doctor_line);
        }
        ...
        holder.doctorLicense.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                RCPhotoPreviewActivity.goActivity(context, introduceDTO.getCertification(), true, false);
            }
        });
    }
    ...
    public class DoctorInfoHolder extends RecyclerView.ViewHolder {
        ...
        //评价星级
        private RCMyRatingBar startView;
        //无评价布局
        ...
        public DoctorInfoHolder(View itemView) {
            ...
            startView = (RCMyRatingBar) itemView.findViewById(R.id.activity_f_d_i_doctor_info_evaluate_start);
            ...
            if (RCFDConfigUtil.getConfig().imageResFDIntroducedTopBg() > 0) {
                evaluateBgImg.setBackgroundResource(RCFDConfigUtil.getConfig().imageResFDIntroducedTopBg());
            }
        }
    }
...
//                "yyyy-MM-dd  HH:mm"));
    ...
    public class CommentHolder extends RecyclerView.ViewHolder {
        ...
        public CommentHolder(View itemView) {
            ...
            startView = (RCMyRatingBar) itemView.findViewById(R.id.activity_f_d_i_comment_start);
        }
    }
    ...
    public void addMyDoctor() {
        ...
        new RCFDDoctorRequestImpl(context).addMyDoctor(introduceDTO.getDoctor_id(), new IRCPostListener() {
            @Override
            public void getDataSuccess() {
                mRcHandler.sendMessage(RCHandler.GETDATA_OK);
                introduceDTO.setFocus(1);
                EventFDAddBean addBean = new EventFDAddBean(EventFDBaseBean.FD_EVENT_TYPE_DO_ADD_MY_DOCTOR);
                addBean.setDoctor_id(introduceDTO.getDoctor_id());
