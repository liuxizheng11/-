                ...
                switch (object.getInt("status")) {
                    ...
                    case RequestCode.STATUS_CODE_LOGIN_OUT:
                        if (RCBaseManage.getInstance().getRequestDataErrorLister() != null) {
                            RCBaseManage.getInstance().getRequestDataErrorLister().error(RequestCode.STATUS_CODE_LOGIN_OUT);
                        }

                        return false;
                    case RequestCode.STATUS_CODE_LOGIN_ERROR:
                        if (RCBaseManage.getInstance().getRequestDataErrorLister() != null) {
                            RCBaseManage.getInstance().getRequestDataErrorLister().error(RequestCode.STATUS_CODE_LOGIN_ERROR);
                        }
                        return false;
                    default:
                        if (RCBaseConfig.isDebug)
